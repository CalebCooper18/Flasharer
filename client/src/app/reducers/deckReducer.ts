...
export const { setDecks, addDeck, deleteDeck, updateDeck, setIsLoading } =
  deckReducer.actions;
...
export function initializeDecks(userDecks: boolean, signal: AbortSignal) {
  return async function (dispatch: ThunkDispatch<unknown, unknown, AnyAction>) {
    try {
      ...
      const allDecks: Deck[] = userDecks
        ? await deckService.getAllUserDecks()
        : await deckService.getAllSharedDecks();
    ...
    try {
      ...
      dispatch(
        createAndDeleteNotification({
          type: 'success',
          message:
            deck.likes < updatedDeck.likes ? 'Deck Liked' : 'Like Removed',
        })
      );
    } catch (error) {
      dispatch(
